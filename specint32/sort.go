...
func siftDown(data []int32, lo, hi, first int) {
	root := lo
	for {
		child := 2*root + 1
		if child >= hi {
...
func heapSort(data []int32, a, b int) {
	first := a
	lo := 0
	hi := b - a
...
func medianOfThree(data []int32, m1, m0, m2 int) {
	if data[m1] < data[m0] {
...
func swapRange(data []int32, a, b, n int) {
	for i := 0; i < n; i++ {
		data[a+i], data[b+i] = data[b+i], data[a+i]
	}
}

func doPivot(data []int32, lo, hi int) (midlo, midhi int) {
	m := lo + (hi-lo)/2
	if hi-lo > 40 {
		s := (hi - lo) / 8
...
func quickSort(data []int32, a, b, maxDepth int) {
	for b-a > 12 {
		if maxDepth == 0 {
			heapSort(data, a, b)
			return
...
func SpecializedSort(data []int32) {
	n := len(data)
	maxDepth := 0
	for i := n; i > 0; i >>= 1 {
		maxDepth++
